...
echo " "
echo " 5. Recon-ng will run with a set of modules that do not require API Keys. For best results spend some time"
...
#theHarvester
theHarvester='/root/Tools/theHarvester/theHarvester.py' #hardcode the path to your local 'theHavester.py' file

# Linkedin2Username
li2u='/root/Tools/linkedin2username/linkedin2username.py' #hardcode the path to your local 'linkedin2username.py' file

#PyMeta
pymeta='/root/Tools/pymeta/pymeta.py' #hardcode the path to your local 'pymeta.py' file

#Sublist3r
sub='/root/Tools/sublist3r/sublist3r.py'

#Gowitness
#gowit='root/Tools/' ---> Only use if command is not executed from CLI. Example would be if you downloadeded pre-built.
...
if [ "$havester" = 'B' ]; then
	# Basic Scan
	echo " "
	echo "${CYAN} [+][+][+]  theHarvester - Basic [+][+][+]${NC}"
	echo " "
	$theHarvester -d "$domain" -b yahoo,bing,hackertarget > harvester_basic_results 2>/dev/null
...
echo "${CYAN} [+][+][+]  MX Records with NSlookup [+][+][+]${NC}"
echo " "
...
echo "${CYAN} [+][+][+]  Sublist3r [+][+][+]${NC}"
echo " "
echo "${CYAN} [+] Command is running and saving output to a file [+]${NC}"
echo " "
...
echo " "
echo "${CYAN} [+][+][+]  GoWitness [+][+][+]${NC}"
echo " "

mkdir /root/Threat_Labs/Projects/$company/Gowitness

gowitness file -F /root/Threat_Labs/Projects/$company/scope.txt -F -P /root/Threat_Labs/Projects/$company/GoWitness
...
if [ "$dnsenum" = 'B' ]; then
	# Basic Scan
	echo " "
	echo "${CYAN} [+][+][+]  DNSenum - Basic [+][+][+]${NC}"
	echo " "
	dnsenum $domain  > dnsenum_basic_results 2>/dev/null
	
else
	echo " "
	echo "${CYAN} [+][+][+]  DNSenum - Advanced [+][+][+]${NC}"
	echo " "
